(function($) {
  ...
  function _canPlayVideo() {
    ...
    if('canPlayType' in video_element && video_element.canPlayType('video/mp4')) {
      return true;
    }
    else {
      return false;
  ...
  $.createVideoHTML = function(uuid, width, height) {
    var video_url = document.location.protocol + '//' + _videos_domain + '/' + uuid + '.mp4'
  ...
  $.videoThumbnailURL = function(uuid) {
    return document.location.protocol + '//s3.amazonaws.com/video_bucket/' + uuid + '.mp4_50_thumb.jpg';
  };
})(jQuery);